...
public class Printer extends PrinterUtils {
			...
			for (int x = -range; x < range + 1; x++) {
				for (int z = -range; z < range + 1; z++) {
					...
					if (shouldClickBlock(targetState, requiredState)) {
						addQueuedPacket(pos, Direction.UP, new Vec3d(pos.getX(), pos.getY(), pos.getZ()), null, false);
	...
	private void swapHandWithSlot(int slot) {
		ItemStack stack = playerEntity.inventory.getInvStack(slot);
		InventoryUtils.setPickedItemToHand(stack, client);
	}
	...
	private int getBlockInventorySlot(Block block) {
    	...
    	for (int slot = 0; slot < inv.getInvSize(); slot++) {
    		if (inv.getInvStack(slot).getItem().equals(block.asItem())) return slot;
		}
