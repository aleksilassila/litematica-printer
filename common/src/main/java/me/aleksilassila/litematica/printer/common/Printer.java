...
import me.aleksilassila.litematica.printer.common.guides.Guides;
...
public class Printer {
    ...
    public boolean onGameTick() {
        ...
        if (!LitematicaMod.PRINT_MODE.getBooleanValue() && !LitematicaMod.PRINT.getKeybind().isPressed())
            return false;
        ...
        for (BlockPos position : positions) {
            ...
            for (Guide guide : guides) {
                if (guide.canExecute(player)) {
                    if (LitematicaMod.DEBUG) System.out.println("Executing " + guide + " for " + state);
    ...
    private List<BlockPos> getReachablePositions() {
        int maxReach = (int) Math.ceil(LitematicaMod.PRINTING_RANGE.getDoubleValue());
        double maxReachSquared = MathHelper.square(LitematicaMod.PRINTING_RANGE.getDoubleValue());
